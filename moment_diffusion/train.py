...
from moment_diffusion.model import ScatteringDenoiser
...
class ScatteringTransformerAdapter(torch.nn.Module):
    ...
    def __init__(self, denoiser, l1_lambda=0.0):
        ...
        self.moment_loss_module = None  # Set externally if enabled
        self.timesteps = 500  # Default, overridden if moment loss enabled
    ...
    def compute_loss(self, noisy_data, true_X, true_E, lw_X, lw_E, unconditioned=False):
        ...
        # --- Moment matching loss (before reshape, while spatial dims are intact) ---
        moment_loss_weighted = torch.tensor(0.0, device=pred.X.device)
        moment_loss_raw_val = 0.0
        if self.moment_loss_module is not None:
            moment_loss_weighted, moment_loss_raw = self.moment_loss_module(
                pred.X, pred.E, noisy_data['node_mask'],
                noisy_data['y_t'], noisy_data['t'], self.timesteps
            )
            moment_loss_raw_val = moment_loss_raw.item()
        ...
        if self.l1_lambda > 0:
            ...
            loss = base_loss + self.l1_lambda * l1_penalty + moment_loss_weighted
        else:
            loss = base_loss + moment_loss_weighted
        ...
        # One-time gradient probe: verify moment loss gradients reach cross-attention (step 1 = after first backward)
        # Safe to remove after confirming on cluster.
        if self.step == 1 and self.moment_loss_module is not None:
            for name, param in self.denoiser.named_parameters():
                if 'cross_attn' in name and param.grad is not None:
                    grad_norm = param.grad.norm().item()
                    print(f"[moment_grad_probe] {name}: grad_norm={grad_norm:.6e}")
                    log_dict[f"grad_probe/{name}"] = grad_norm
            print("[moment_grad_probe] Done. Remove this check after confirming.")
        wandb.log(log_dict)
        self.step += 1
...
class ScatteringGraphDIT(GraphDITMolecularGenerator):
    ...
    def _initialize_model(self, model_class, checkpoint=None):
        ...
        # Cross-attention regularization (helps prevent overfitting to scattering moments)
        ...
        # Initialize moment matching loss if enabled in config
        moment_cfg = self.config.get('moment_loss', {})
        if moment_cfg.get('enabled', False):
            from moment_diffusion.moment_loss import MomentMatchingLoss
            self.model.moment_loss_module = MomentMatchingLoss(
                num_atom_types=getattr(self, 'num_atom_types', 16),
                J=self.config.get('scattering', {}).get('J', 4),
                num_moments=self.config.get('scattering', {}).get('num_moments', 4),
                lambda_base=moment_cfg.get('lambda_base', 0.1),
                t_weighting=moment_cfg.get('t_weighting', True),
            ).to(self.device)
            self.model.timesteps = self.timesteps
            print(f"Moment matching loss enabled: lambda_base={moment_cfg.get('lambda_base', 0.1)}")
    ...
    def _compute_generation_metrics(self, n_samples=100):
        """Generate molecules and compute validity/uniqueness."""
        ...
        validity = 100.0 * len(valid_smiles) / len(generated_smiles) if generated_smiles else 0.0
        unique_smiles = list(set(valid_smiles))
        uniqueness = 100.0 * len(unique_smiles) / len(valid_smiles) if valid_smiles else 0.0
